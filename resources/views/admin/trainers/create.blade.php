    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="mb-6">
            <div class="flex items-center space-x-3 mb-2">
                <a href="{{ route('admin.trainers.index') }}" class="text-gray-600 hover:text-gray-900">
                    <i class="fas fa-arrow-left"></i>
                </a>
                <h1 class="text-3xl font-bold text-gray-900">Add New Trainer</h1>
            </div>
            <p class="text-sm text-gray-600">Create a new trainer account</p>
        </div>

        <form action="{{ route('admin.trainers.store') }}" method="POST" class="space-y-6">
            @csrf
                ...
                <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="address" id="address" rows="3"
                                  class="w-full px-4 py-3 glass-card rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 transition-all">{{ old('address') }}</textarea>
                    </div>
                </div>
            </div>
            ...
            <div class="glass-card rounded-xl p-6">
                ...
                <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                    <div>
                        ...
                        <input type="text" name="specialization" id="specialization" required value="{{ old('specialization') }}"
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="certifications" id="certifications" rows="3"
                                  class="w-full px-4 py-3 glass-card rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 transition-all">{{ old('certifications') }}</textarea>
                    </div>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="bio" id="bio" rows="4"
                                  class="w-full px-4 py-3 glass-card rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 transition-all">{{ old('bio') }}</textarea>
                    </div>
                </div>
            </div>
                ...
                <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                    <div>
                        ...
                        <input type="password" name="password" id="password" required
                               class="w-full px-4 py-3 glass-card rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 transition-all @error('password') ring-2 ring-red-500 @enderror">
                        @error('password')
                            <p class="mt-2 text-sm text-red-600 flex items-center">
                                <i class="fas fa-exclamation-circle mr-1"></i>{{ $message }}
                            </p>
